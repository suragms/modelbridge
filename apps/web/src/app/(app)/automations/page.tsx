    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[var(--muted-foreground)]">
          Event-driven triggers and workflow actions with auditable execution history.
        </p>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-base">Active Automations</CardTitle></CardHeader>
          ...
          {list.map((a) => (
            <div key={String(a.id)} className="rounded border px-3 py-2 text-sm">
              ...
              <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                ...
                {Boolean(a.requires_approval) && " (approval required)"}
              </p>
            </div>
          ))}
        </CardContent>
